...
`Os números da unidade Francy {{UNIDADE}} são {{TELEFONE1}} e {{TELEFONE2}}📱 Ambos os números também são WhatsApp e você pode entrar em contato por mensagem por eles.`
  ...
  // ---------- Confirmação (usada antes de ações que apagam algo) ----------

  const confirmOverlay = document.getElementById('confirm-overlay');
  const confirmMensagemEl = document.getElementById('confirm-mensagem');
  const confirmCancelarBtn = document.getElementById('confirm-cancelar');
  const confirmConfirmarBtn = document.getElementById('confirm-confirmar');
  let confirmResolve = null;

  // Retorna uma Promise<boolean> — true se confirmou, false se cancelou
  // (ou fechou clicando fora / Esc). Uso: const ok = await confirmarAcao('...');
  function confirmarAcao(mensagem) {
    return new Promise((resolve) => {
      confirmMensagemEl.textContent = mensagem;
      confirmResolve = resolve;
      confirmOverlay.classList.add('show');
      setTimeout(() => confirmConfirmarBtn.focus(), 30);
    });
  }
  ...
  confirmCancelarBtn.addEventListener('click', () => fecharConfirm(false));
  confirmConfirmarBtn.addEventListener('click', () => fecharConfirm(true));
  confirmOverlay.addEventListener('click', (e) => {
    if (e.target === confirmOverlay) fecharConfirm(false);
  });
  ...
  function addAtendenteRow(checked, nomeInicial) {
    ...
    // Clicar (ou navegar por Tab até) o campo de nome já seleciona esse
    // atendente — não precisa acertar o botão de rádio pequenininho.
    ...
    row.querySelector('.atendente-remove').addEventListener('click', async () => {
      const nomeDigitado = nome.value.trim();
      const ok = await confirmarAcao(
        nomeDigitado
          ? 'Remover o atendente "' + nomeDigitado + '"?'
          : 'Remover este atendente?'
      );
      if (!ok) return;
  ...
  function addItemRow() {
    ...
    row.querySelector('.item-remove').addEventListener('click', async () => {
      const descDigitada = row.querySelector('.item-desc').value.trim();
      const ok = await confirmarAcao(
        descDigitada
          ? 'Remover o item "' + descDigitada + '" do pedido?'
  ...
  clearResumoBtn.addEventListener('click', async () => {
    const ok = await confirmarAcao('Iniciar um novo pedido? Isso vai apagar todos os itens e a taxa de entrega atuais.');
    if (!ok) return;
  ...
  function buildResumo() {
    ...
    const linhas = items.map((it) => {
      ...
      const desc = it.descRaw ? ' ' + it.descRaw : '';
  ...
  pinBtn.addEventListener('click', async () => {
    ...
    pinBtn.title = pinned
      ? 'Fixado — clique para não ficar mais sempre por cima'
  ...
  // ---------- Validar Receitas (abre o site do ITI no navegador) ----------

  const validarReceitasBtn = document.getElementById('btn-validar-receitas');

  validarReceitasBtn.addEventListener('click', async () => {
    try {
      if (window.francy && typeof window.francy.abrirValidarReceitas === 'function') {
        await window.francy.abrirValidarReceitas();
      }
    } catch (err) {
      showToast('Não foi possível abrir o navegador.', true);
    }
  });

  // ---------- ProDoctor Medicamentos (abre a referência no navegador) ----------

  const prodoctorBtn = document.getElementById('btn-prodoctor');

  prodoctorBtn.addEventListener('click', async () => {
    try {
      if (window.francy && typeof window.francy.abrirProDoctor === 'function') {
        await window.francy.abrirProDoctor();
      }
    } catch (err) {
      showToast('Não foi possível abrir o navegador.', true);
    }
  ...
  // ---------- Parcial de vendas (tela própria, protegida por senha) ----------

  const parcialBtn = document.getElementById('btn-parcial');

  async function abrirTelaParcial() {
    try {
      if (window.francy && typeof window.francy.abrirParcial === 'function') {
        await window.francy.abrirParcial();
      }
    } catch (err) {
      showToast('Não foi possível abrir a tela do Parcial.', true);
    }
  }

  parcialBtn.addEventListener('click', () => requestPassword(abrirTelaParcial));

  // ---------- Cálculo de Dispensação (controlados e antibióticos) ----------
  //
  // Prazos máximos de tratamento conforme Portaria SVS/MS nº 344/98:
  //   C1 (Receituário de Controle Especial): 60 dias — 180 dias para
  //     anticonvulsivante/antiparkinsoniano (sem justificativa) ou qualquer
  //     substância C1 com justificativa assinada pelo prescritor.
  //   B1 (Notificação Azul — psicotrópicos): 60 dias.
  //   B2 (Notificação Azul — anorexígenos): 30 dias — 60 dias apenas para
  //     sibutramina (RDC 133/2016).
  //   A (Notificação Amarela — entorpecentes/psicotrópicos): 30 dias.
  // Antibióticos (RDC nº 20/2011): a dispensação deve atender exatamente à
  //   quantidade prescrita para o tratamento; só se libera quantidade maior
  //   quando não existe apresentação comercial com a quantidade exata —
  //   nunca se libera menos do que o necessário.

  const dispBtn = document.getElementById('btn-dispensacao');
  const dispOverlay = document.getElementById('dispensacao-overlay');
  const dispTipo = document.getElementById('disp-tipo');
  const dispControladosFields = document.getElementById('disp-controlados-fields');
  const dispAntibioticoFields = document.getElementById('disp-antibiotico-fields');
  const dispC1Extra = document.getElementById('disp-c1-extra');
  const dispC1Prazo = document.getElementById('disp-c1-prazo');
  const dispB2Extra = document.getElementById('disp-b2-extra');
  const dispB2Prazo = document.getElementById('disp-b2-prazo');
  const dispDiasInformados = document.getElementById('disp-dias-informados');
  const dispQtdDose = document.getElementById('disp-qtd-dose');
  const dispVezesDia = document.getElementById('disp-vezes-dia');
  const dispUnidCaixa = document.getElementById('disp-unid-caixa');
  const dispAtbForma = document.getElementById('disp-atb-forma');
  const dispAtbSolidoFields = document.getElementById('disp-atb-solido-fields');
  const dispAtbLiquidoFields = document.getElementById('disp-atb-liquido-fields');
  const dispAtbQtdDose = document.getElementById('disp-atb-qtd-dose');
  const dispAtbUnidCaixa = document.getElementById('disp-atb-unid-caixa');
  const dispAtbMlDose = document.getElementById('disp-atb-ml-dose');
  const dispAtbMlFrasco = document.getElementById('disp-atb-ml-frasco');
  const dispAtbVezesDia = document.getElementById('disp-atb-vezes-dia');
  const dispAtbDias = document.getElementById('disp-atb-dias');
  const dispCalcularBtn = document.getElementById('disp-calcular');
  const dispResultado = document.getElementById('disp-resultado');
  const dispFecharBtn = document.getElementById('disp-fechar');

  function openDispModal() {
    dispResultado.classList.remove('show', 'erro');
    dispResultado.innerHTML = '';
    dispOverlay.classList.add('show');
  }
  function closeDispModal() {
    dispOverlay.classList.remove('show');
  }

  dispBtn.addEventListener('click', openDispModal);
  dispFecharBtn.addEventListener('click', closeDispModal);
  dispOverlay.addEventListener('click', (e) => {
    if (e.target === dispOverlay) closeDispModal();
  });

  function atualizarCamposDispTipo() {
    const tipo = dispTipo.value;
    const isAntibiotico = tipo === 'antibiotico';
    dispControladosFields.style.display = isAntibiotico ? 'none' : '';
    dispAntibioticoFields.style.display = isAntibiotico ? '' : 'none';
    dispC1Extra.style.display = tipo === 'c1' ? '' : 'none';
    dispB2Extra.style.display = tipo === 'b2' ? '' : 'none';
    dispResultado.classList.remove('show', 'erro');
  }
  dispTipo.addEventListener('change', atualizarCamposDispTipo);
  atualizarCamposDispTipo();

  function atualizarFormaAntibiotico() {
    const isLiquido = dispAtbForma.value === 'liquido';
    dispAtbSolidoFields.style.display = isLiquido ? 'none' : '';
    dispAtbLiquidoFields.style.display = isLiquido ? '' : 'none';
  }
  dispAtbForma.addEventListener('change', atualizarFormaAntibiotico);
  atualizarFormaAntibiotico();

  function parseNumDisp(str) {
    const n = parseFloat((str || '').toString().trim().replace(',', '.'));
    return isNaN(n) ? 0 : n;
  }
  ...
  function mostrarResultadoDisp(html, isErro) {
    dispResultado.innerHTML = html;
    dispResultado.classList.toggle('erro', !!isErro);
    dispResultado.classList.add('show');
  }

  dispCalcularBtn.addEventListener('click', () => {
    const tipo = dispTipo.value;

    // ---- Antibióticos ----
    if (tipo === 'antibiotico') {
      const forma = dispAtbForma.value;
      const vezesDia = parseNumDisp(dispAtbVezesDia.value);
      const dias = parseNumDisp(dispAtbDias.value);

      if (vezesDia <= 0 || dias <= 0) {
        mostrarResultadoDisp('Preencha "vezes ao dia" e "dias de tratamento" com valores maiores que zero.', true);
        return;
      }

      if (forma === 'liquido') {
        const mlDose = parseNumDisp(dispAtbMlDose.value);
        const mlFrasco = parseNumDisp(dispAtbMlFrasco.value);
        if (mlDose <= 0 || mlFrasco <= 0) {
          mostrarResultadoDisp('Preencha "mL por dose" e "mL que o frasco vem" com valores maiores que zero.', true);
          return;
        }
        const totalMl = mlDose * vezesDia * dias;
        const frascos = Math.ceil(totalMl / mlFrasco);
        mostrarResultadoDisp(
          '<strong>' + frascos + ' frasco' + (frascos > 1 ? 's' : '') + '</strong><br>' +
          'Total necessário: ' + formatarNumero(totalMl) + ' mL para ' + dias + ' dia' + (dias > 1 ? 's' : '') + ' de tratamento.<br>' +
          '<span class="disp-nota">RDC nº 20/2011: a dispensação deve atender exatamente à quantidade prescrita. ' +
          'Se não houver apresentação com a quantidade exata, libera-se a mais próxima acima do necessário — nunca abaixo.</span>'
        );
      } else {
        const qtdDose = parseNumDisp(dispAtbQtdDose.value);
        const unidCaixa = parseNumDisp(dispAtbUnidCaixa.value);
        if (qtdDose <= 0 || unidCaixa <= 0) {
          mostrarResultadoDisp('Preencha "quantidade por dose" e "unidades por caixa" com valores maiores que zero.', true);
          return;
        }
        const totalUnidades = qtdDose * vezesDia * dias;
        const caixas = Math.ceil(totalUnidades / unidCaixa);
        mostrarResultadoDisp(
          '<strong>' + caixas + ' caixa' + (caixas > 1 ? 's' : '') + '</strong><br>' +
          'Total necessário: ' + formatarNumero(totalUnidades) + ' unidades para ' + dias + ' dia' + (dias > 1 ? 's' : '') + ' de tratamento.<br>' +
          '<span class="disp-nota">RDC nº 20/2011: a dispensação deve atender exatamente à quantidade prescrita. ' +
          'Se não houver apresentação com a quantidade exata, libera-se a mais próxima acima do necessário — nunca abaixo.</span>'
        );
      }
    ...
    // ---- Controlados (C1 / B1 / B2 / A) ----
    const qtdDose = parseNumDisp(dispQtdDose.value);
    const vezesDia = parseNumDisp(dispVezesDia.value);
    const unidCaixa = parseNumDisp(dispUnidCaixa.value);
    const diasInformadosRaw = dispDiasInformados.value.trim();
    const diasInformados = diasInformadosRaw ? parseNumDisp(diasInformadosRaw) : null;

    if (qtdDose <= 0 || vezesDia <= 0 || unidCaixa <= 0) {
      mostrarResultadoDisp('Preencha "quantidade por dose", "vezes ao dia" e "unidades por caixa" com valores maiores que zero.', true);
      return;
    }

    let diasMaximoCategoria;
    let nomeCategoria;
    if (tipo === 'c1') {
      diasMaximoCategoria = dispC1Prazo.value === '60' ? 60 : 180;
      nomeCategoria = 'C1 — Receituário de Controle Especial';
    } else if (tipo === 'b1') {
      diasMaximoCategoria = 60;
      nomeCategoria = 'B1 — Notificação Azul (psicotrópicos)';
    } else if (tipo === 'b2') {
      diasMaximoCategoria = dispB2Prazo.value === '60-sibutramina' ? 60 : 30;
      nomeCategoria = 'B2 — Notificação Azul (anorexígenos)';
    } else {
      diasMaximoCategoria = 30;
      nomeCategoria = 'A — Notificação Amarela';
    }

    let diasUsados = diasMaximoCategoria;
    let origemDias = 'limite máximo da categoria';
    if (diasInformados && diasInformados > 0) {
      if (diasInformados <= diasMaximoCategoria) {
        diasUsados = diasInformados;
        origemDias = 'dias informados na receita';
      } else {
        diasUsados = diasMaximoCategoria;
        origemDias = 'os dias informados excedem o limite da categoria — usando o máximo permitido';
      }
    }

    const totalUnidades = qtdDose * vezesDia * diasUsados;
    const caixas = Math.ceil(totalUnidades / unidCaixa);

    mostrarResultadoDisp(
      '<strong>' + caixas + ' caixa' + (caixas > 1 ? 's' : '') + '</strong><br>' +
      nomeCategoria + ' — ' + diasUsados + ' dia' + (diasUsados > 1 ? 's' : '') + ' de tratamento (' + origemDias + ').<br>' +
      'Total necessário: ' + formatarNumero(totalUnidades) + ' unidades.<br>' +
      '<span class="disp-nota">Confira sempre a prescrição, a apresentação disponível e critérios técnicos ' +
      '(ex.: farmacoterapia prévia do paciente) antes de liberar.</span>'
    );
  });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    else if (dispOverlay.classList.contains('show')) closeDispModal();
    else if (confirmOverlay.classList.contains('show')) fecharConfirm(false);
  });
